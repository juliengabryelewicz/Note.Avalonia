...
namespace Note.AppCtx
{
    public class LoggingCtx
    {
        ...
        private static ILogger InitLogging(string ns)
        {
            ...
            string sql = Regex.Replace((Environment.GetEnvironmentVariable($"{nameof(Note)}_{ns}_LOGGING".ToUpper())
                ?? Environment.GetEnvironmentVariable($"{nameof(Note)}_DEFAULT_LOGGING".ToUpper())
                ?? String.Empty),
                "^$", "None");
            ...
            if (sql == "None") {
                ...
                string appSqlFile = Path.Combine(Program.ConfigCtx.LogDirectory, $"{nameof(Note)}-{ns}.log");
